...
#define HASH_UPD(x) Sha256_Update2(&hash, (const Byte *)&x, sizeof(x));
...
void CRandomGenerator::Init()
{
  ...
  #else
  {
    ...
    if (hModule)
    {
      ...
      if (my_RtlGenRandom)
      {
        ...
        {
          numIterations = kNumIterations_Small;
          Sha256_Update2(&hash, buf, kBufSize);
        }
      }
      ::FreeLibrary(hModule);
    }
  ...
  */
  ...
  do
  {
    ...
    {
      Sha256_Final2(&hash, _buff);
      Sha256_Init2(&hash);
      Sha256_Update2(&hash, _buff, SHA256_DIGEST_SIZE);
    }
  }
  while (--numIterations);
